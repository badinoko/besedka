...
### 📍 ЭТАП 1: КРИТИЧЕСКИЕ ИСПРАВЛЕНИЯ СЧЕТЧИКОВ И НАВИГАЦИИ

#### 1.1 🔧 ИСПРАВЛЕНИЕ СЧЕТЧИКОВ УВЕДОМЛЕНИЙ
**ПРОБЛЕМА:** Пользователь видит "6" в UI, но в БД 0 непрочитанных уведомлений у owner

**ПОДПУНКТЫ:**
- [ ] **1.1.1** Аудит логики счетчика в `core/context_processors/navigation.py`
- [ ] **1.1.2** Проверка шаблона счетчика в `includes/navigation.html`  
- [ ] **1.1.3** Тестирование обновления счетчика после прочтения уведомлений
- [ ] **1.1.4** Исправление JavaScript обработчиков для real-time обновления
- [ ] **1.1.5** Проверка AJAX запросов для пометки прочитанными
- [ ] **1.1.6** Валидация через тестирование всех ролей (owner, admin, user)

**ФАЙЛЫ К ПРОВЕРКЕ:**
```
core/context_processors/navigation.py
templates/includes/navigation.html  
static/js/ (файлы с уведомлениями)
users/views.py (NotificationListView)
templates/users/notification_list.html
```

**КРИТЕРИИ ГОТОВНОСТИ:**
- Счетчик показывает точное количество непрочитанных
- Real-time обновление при прочтении/получении уведомлений
- Работает для всех ролей пользователей

#### 1.2 🎨 УНИФИКАЦИЯ ЗАГОЛОВКОВ И НАВИГАЦИИ
**ПРОБЛЕМА:** "Двойные заголовки" по всей платформе из-за глобальной навигации + кастомных заголовков

**ПОДПУНКТЫ:**
- [ ] **1.2.1** Анализ всех кастомных заголовков (чат, галерея, новости, гроурепорты)
- [ ] **1.2.2** Выбор эталонного стиля (градиентные заголовки гроурепортов/чата)
- [ ] **1.2.3** Создание универсального компонента `includes/unified_header.html`
- [ ] **1.2.4** Рефакторинг `templates/base.html` под новую структуру
- [ ] **1.2.5** Адаптация всех модулей под единый заголовок:
...
- [ ] **1.2.6** Обновление админских шаблонов (`templates/admin/base_site.html`)
- [ ] **1.2.7** Унификация хлебных крошек (выбор одного стиля: Bootstrap vs кастомный)
...
#### 1.3 📱 РЕФАКТОРИНГ ВСТРОЕННОГО CSS/JS
**ПРОБЛЕМА:** Massive встроенные `<style>` и `<script>` блоки ухудшают поддерживаемость

**ПОДПУНКТЫ:**
- [ ] **1.3.1** Аудит всех шаблонов с встроенными стилями:
  ...
  - `templates/growlogs/list_growdiaries_inspired.html`
  - `templates/users/profile.html`
- [ ] **1.3.2** Создание модульных CSS файлов:
  - `static/css/store.css`
  - `static/css/news.css`
  - `static/css/growlogs.css`
  - `static/css/profile.css`
- [ ] **1.3.3** Вынос JavaScript в отдельные файлы:
  - `static/js/cart-manager.js`
  - `static/js/store-ajax.js`
  - `static/js/news-interactions.js`
  - `static/js/growlogs-filters.js`
- [ ] **1.3.4** Обновление версионности статических файлов
- [ ] **1.3.5** Тестирование загрузки и кеширования

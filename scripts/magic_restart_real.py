...
"""
    ...
    print(f"🔗 Интегрированный чат: http://127.0.0.1:8001/chat/integrated/")
    ...
    try:
        ...
        if result.returncode == 0:
            print("✅ Авторизация в Rocket.Chat выполнена автоматически!")
        else:
            print("⚠️ Автоматическая авторизация не удалась, выполните вручную:")
            print("   python scripts/auto_login_rocketchat.py")

    except Exception as e:
        print(f"⚠️ Ошибка автоматической авторизации: {e}")
        print("   Выполните авторизацию вручную: python scripts/auto_login_rocketchat.py")

    print("\n" + "="*60)
    print("🚀 ВСЁ ГОТОВО! СИСТЕМА ПОЛНОСТЬЮ ЗАПУЩЕНА И АВТОРИЗОВАНА")
    print("="*60)
